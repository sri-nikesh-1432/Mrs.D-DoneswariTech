    ...
    <div className="h-screen w-full flex flex-col bg-sky-50 overflow-hidden">
      ...
      <header className="glass-nav px-4 py-3 flex items-center justify-between z-20 border-b border-sky-200/60 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-7 h-7 rounded-full bg-sky-200 text-sky-700 text-xs font-medium flex items-center justify-center">
            U
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex relative overflow-hidden">
            ...
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "ai" ? "justify-start" : "justify-end"}`}>
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                  m.role === "ai"
                    ? "bg-white/70 text-sky-900 border border-sky-100 shadow-sm"
                    : "bg-sky-100 text-sky-800"
                }`}>
                  {m.role === "ai" ? <span className="text-[10px] text-sky-400 font-medium block mb-0.5">Mrs.D</span> : null}
                  {m.content}
                </div>
              </div>
            ))}
        ...
        {/* Right memory panel */}
        <aside className="w-72 bg-white/60 border-l border-sky-200/50 p-4 flex flex-col gap-4 overflow-y-auto hidden sm:flex">
          <div className="text-xs font-semibold text-sky-500 uppercase tracking-wider">Caller Memory</div>
          <div className="flex flex-col gap-2.5">
            {MEMORY_FIELDS.map((f) => (
              <div key={f} className="flex justify-between text-sm">
                <span className="text-sky-500">{f}</span>
                <span className="text-sky-900 font-medium">{memory[f] || "—"}</span>
              </div>
            ))}
          </div>
        </aside>
      ...
      {/* Input bar */}
      <div className="flex-shrink-0 border-t border-sky-200/50 bg-white/70 px-4 py-3 flex items-center gap-3 z-20">
        <button
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${microOn ? "bg-red-100 text-red-500 ring-2 ring-red-300" : "bg-sky-100 text-sky-500 hover:bg-sky-200"}`}
          onClick={toggleMic}
        >
          {microOn ? (
