"""
Groq Whisper Speech-to-Text service (active app package).

Transcribes audio captured by the frontend's real-time VAD + MediaRecorder.
Language is NOT hardcoded — Whisper Large V3 Turbo auto-detects Telugu,
Hindi, Tamil, Kannada, Malayalam, English, etc., so ANY voice works.
"""
...
async def transcribe_audio(audio_bytes: bytes, filename: str = "audio.webm") -> dict:
    ...
    Returns dict: {"text": str, "language": "te"|"hi"|"en"|...}
    """
    if not audio_bytes:
    ...
    try:
        transcription = await client.audio.transcriptions.create(
            ...
            response_format="verbose_json",  # gives us the language field
        )
